...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyIdPageThenGetVacancyPageWithId() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    @Test
    public void whenRequestEditVacancyPageAndUpdateVacancyThenRedirectToVacanciesPage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenRequestVacancyDeleteThenCanNotGetVacancyPageWithDeletedIdAndGet404PageWithErrorMessage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        ...
        var model = new ConcurrentModel();
        var view1 = vacancyController.delete(model, vacancy.getId());
        var view2 = vacancyController.getById(model, vacancy.getId());
        ...
        assertThat(actualErrorMessage).isEqualTo("Вакансия с указанным идентификатором не найдена");
    }
}