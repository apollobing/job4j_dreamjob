...
class CandidateControllerTest {
    ...
    public void initServices() {
        ...
        testFile = new MockMultipartFile("testFile.pdf", new byte[]{1, 2, 3});
    }
    ...
    @Test
    public void whenRequestCandidateIdPageThenGetCandidatePageWithId() {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
    ...
    @Test
    public void whenRequestEditCandidatePageAndUpdateCandidateRedirectToCandidatesPage() {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenRequestCandidateDeleteThenCanNotGetCandidatePageWithDeletedIdAndGet404PageWithErrorMessage() {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        ...
        var model = new ConcurrentModel();
        var view1 = candidateController.delete(model, candidate.getId());
        var view2 = candidateController.getById(model, candidate.getId());
        var actualErrorMessage = model.getAttribute("message");

        assertThat(view1).isEqualTo("redirect:/candidates");
        assertThat(view2).isEqualTo("errors/404");
        assertThat(actualErrorMessage).isEqualTo("Кандидат с указанным идентификатором не найден");
    }
}